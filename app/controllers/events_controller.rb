class EventsController < ApplicationController
  ...
  def index
    ...
    @events_now = Event.where('datetime BETWEEN ? AND ?', DateTime.now, DateTime.now - 5.hours).all.sort_by! {|event| event.datetime}
  ...
  def send_texts(event, flag)
    ...
    @message = "#{flag == "n" ? "New" : "Updated"} Event: #{event.name}, D: #{l event.datetime - 7.hours, :format => :long}, L: #{event.location}. Reply \"#{event.id} y\" to accept, \"#{event.id} n\" to decline"
    @users.each do |user|
